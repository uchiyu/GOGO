...
public class User_s13t208_02 extends GogoCompSub {
  public static int decreasePoint = 0;
  //====================================================================
  //  コンストラクタ
  //====================================================================

  public User_s13t208_02(GamePlayer player) {
    super(player);
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        if ( check_run(cell, mycolor*-1, i, j, 4) ) {
          ...
          switch ( k ) {
            case 2: values[i][j] += 12000; break;
            case 1: values[i][j] += 1000; break;
            case 0: values[i][j] += 1; break;
          }
          if ( taken_form( cell, mycolor, i, j ) ) { values[i][j] -= 2000; }
          values[i][j] -= decreasePoint;
        }
        ...
        if ( check_jump( cell, mycolor*-1, i, j, 4) ) {
          ...
          switch ( k ) {
            case 2: values[i][j] += 12000; break;
            case 1: if ( check_rem(cell, mycolor*-1, i, j) ) { values[i][j] += 4000; } break;
            case 0: values[i][j] += 1; break;
          }
          if ( taken_form( cell, mycolor, i, j ) ) { values[i][j] -= 2000; }
        }
        ...
        //相手の石を取る 8個目
        if ( check_rem(cell, mycolor*-1, i, j) && mystone == 6 ) { values[i][j] += 10000; }

        // 自分の四連を作る → 600;
        if ( check_run(cell, mycolor, i, j, 4) ) {
          int k = checkEdge( cell, mycolor, i, j, 4);
          switch ( k ) {
            case 2: values[i][j] += 8000; break;
            case 1: values[i][j] += 1000; break;
            case 0: values[i][j] += 1; break;
          }
          values[i][j] -= decreasePoint;
        ...
        if ( check_jump( cell, mycolor, i, j, 4) )  {
          ...
          switch ( k ) {
            case 2: values[i][j] += 8000; break;
            case 1: values[i][j] += 4000; break;
            case 0: values[i][j] += 1; break;
          }
        }

        // 自分の石を守る → 550;
        if ( check_rem(cell, mycolor, i, j) ) { values[i][j] += 2500; }

        // 相手の石を取る → 520;
        if ( check_rem(cell, mycolor*-1, i, j) ) { values[i][j] += 2000; }
  ...
  boolean check_run_dir(int[][] board, int color, int i, int j, int dx, int dy, int len) {
    ...
    for ( int k = 1; k <= num; k++ ) {
      int x = i+k*-dx;
      int y = j+k*-dy;
      if ( check_out(x, y) ) { decreasePoint += 100; }
      else if ( board[x][y] != 0 ) { decreasePoint += 50; }
